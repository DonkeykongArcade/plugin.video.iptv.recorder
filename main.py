...
DB_PATH = os.path.join(USER_DATA_DIR, 'sxmltv.db')
DB_PATH2 = xbmcaddon.Addon().getAddonInfo("path")
xbmc.log("LOGGIN STUFF", xbmc.LOGINFO)
xbmc.log(DB_PATH, xbmc.LOGINFO)
xbmc.log(DB_PATH2, xbmc.LOGINFO)
...
def play_channel(channelname):
    channelname = decode_string(channelname)
...
def record_once(programmeid, channelid, channelname, do_refresh=True, watch=False, remind=False):
    channelid = decode_string(channelid)
    channelname = decode_string(channelname)
    start = None
    stop = None
...
def record_one_time( channelname):
    ...
    name = decode_string(xbmcgui.Dialog().input("Rule Name"))
...
def record_epg(channelname, name, start, stop):
    ...
    start = get_utc_from_string(decode_string(start))
    stop = get_utc_from_string(decode_string(stop))
...
def record_daily(channelid, channelname, title, start, stop):
    channelid = decode_string(channelid)
    channelname = decode_string(channelname)
    title = decode_string(title)
    title = decode_string(xbmcgui.Dialog().input(_("% is Wildcard"), title))
...
def record_always_search(channelid, channelname):
    ...
    title = decode_string(xbmcgui.Dialog().input("IPTV Recorder: " + _("Title Search (% is wildcard)?")))
    if not title:
        return
...
def record_always_search_plot(channelid, channelname):
    ...
    description = decode_string(xbmcgui.Dialog().input("IPTV Recorder: " + _("Plot Search (% is wildcard)?")))
    if not description:
        return
...
def watch_always_search(channelid, channelname):
    ...
    title = decode_string(xbmcgui.Dialog().input("IPTV watcher: " + _("Title Search (% is wildcard)?")))
    if not title:
        return
...
def watch_always_search_plot(channelid, channelname):
    ...
    description = decode_string(xbmcgui.Dialog().input("IPTV watcher: " + _("Plot Search (% is wildcard)?")))
    if not description:
        return
...
def remind_always_search(channelid, channelname):
    ...
    title = decode_string(xbmcgui.Dialog().input("IPTV reminder: " + _("Title Search (% is wildcard)?")))
    if not title:
        return
...
def remind_always_search_plot(channelid, channelname):
    ...
    description = decode_string(xbmcgui.Dialog().input("IPTV reminder: " + _("Plot Search (% is wildcard)?")))
    if not description:
        return
...
def search_title_input(title):
    ...
    if title == "title":
        title = ""
    d = xbmcgui.Dialog()
    what = decode_string(d.input(_("Search Title"), title))
    #log(what)
    if not what:
        return
    searches[what] = ''
...
def search_plot_input(plot):
    ...
    if plot == "plot":
        plot = ""
    d = xbmcgui.Dialog()
    what = decode_string(d.input(_("Search Plot"), plot))
    if not what:
        return
    searches[what] = ''
...
def delete_search_categories(categories):
    categories = decode_string(categories)
...
def listing(programmes, scroll=False, channelname=None):
    if channelname:
        channelname = urllib.unquote_plus(decode_string(channelname))
...
def group(channelgroup=None,section=None):
    if channelgroup:
        channelgroup=decode_string(channelgroup)
...
def recordings():
    ...
    start_items = sorted(zip(starts,items))
    ...
    for x in ["1","2"]:
        ...
        if path:
            ...
            if "m3u8" in path.lower():
                ...
                log(encoding)
                data = decode_string(data, encoding['encoding'])
